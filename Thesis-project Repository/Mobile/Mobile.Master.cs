...
namespace Thesis_project_Repository.Mobile
{
    public partial class Mobile : System.Web.UI.MasterPage
    {
        ...
        protected string CheckAuthenticationStatus(string username)
        {
            const string query = "SELECT * FROM authentication WHERE username = @username;";
        ...
        protected string CheckAuthenticationMethod(string username)
        {
            const string query = "SELECT * FROM authentication WHERE username = @username;";
                    ...
                    if (reader.Read())
                    {
                        ...
                        if (!reader.GetString(2).Equals("A"))
                        {
                            ...
                            else
                            {
                                ...
                                var authstatus = CheckAuthenticationStatus(loginUserName.Text);
                                if (authstatus == "N")
                                {
                                    ...
                                    if (acctype.Equals("P"))
                                    {
                                        Response.Redirect("../Mobile/ProfessorFiles/ProfessorHome", false);
                                    }
                                    else if (acctype.Equals("S"))
                                    {
                                        Response.Redirect("../Mobile/StudentFiles/StudentHomePage", false);
                                    }
                                    else if (acctype.Equals("V"))
                                    {
                                        Response.Redirect("../Mobile/Viewer/ViewerHome", false);
                                    }
                                }
                                else
                                {
                                    ...
                                    var result = _databaseMethods.UpdateAuthentication(loginUserName.Text, randomString);
                                    var authenticationMethod = CheckAuthenticationMethod(loginUserName.Text);
                                    if (authenticationMethod == "M")
                                    {
                                        bool sent = GetPhoneNumber(randomString);

                                        MultiView1.ActiveViewIndex = 5;
                                    }
                                    else
                                    {
                                        var message = EmailBodyForAuthNumber(randomString);
                                        bool email = SendEmail(loginUserName.Text, "Two-factor Authentication Code", message);
                                        MultiView1.ActiveViewIndex = 5;
                                    }
                                }
                            }
                        ...
                        else
                        {
                            ...
                            Response.Redirect("../Mobile/Admin/AdminHomePage", false);
                        }
                        Session["username"] = loginUserName.Text;
                    }
                }
        ...
        protected string EmailBodyForAuthNumber(int rdmString)
        {
            var message = " <h2>Your two-factor authentication number is:" + rdmString + "</h2>";
        ...
        {
            var messageFrom = new MailAddress("ajain5@ilstu.edu", "ITDepartment");
        ...
        protected void ValidateAuthentication(object sender, EventArgs e)
        {
            var username = Session["username"].ToString();
            var randomString = Convert.ToInt32(authCode.Text);
            var acctype = Session["acctype"].ToString();
            const string queryString1 = "SELECT * FROM AUTHENTICATION WHERE USERNAME = @username AND RANDOMNUMBER = @RANDOMNUMBER;";

            using (var connection = new SqlConnection(ConnectionString))
            {
                var command = new SqlCommand(queryString1, connection);
                command.Parameters.AddWithValue("@username", username);
                command.Parameters.AddWithValue("@RANDOMNUMBER", randomString);
                try
                {
                    connection.Open();
                    var reader = command.ExecuteReader();
                    ...
                    {
                        var usernameDB = reader.GetString(0);
                        var randomnumberDB = reader.GetInt32(2);
                        if (username.Equals(usernameDB) && randomString.Equals(randomnumberDB))
                        {
                            if (acctype.Equals("P"))
                            {
                                Response.Redirect("../Mobile/ProfessorFiles/ProfessorHome", false);
                    ...
                    {
                        //What is the use of false here?
                        Response.Redirect("../Mobile/Default?ia=5", false);
                        authvalidateResult.Text = "Two-factor authentication code is invalid!!";
        ...
        protected bool GetPhoneNumber(int ranonNumber)
        {
            //    const string query = "SELECT * FROM logininfo WHERE username = @username;";
            var username = Session["username"].ToString();
            var Carrier = "";
            var FpAccntType = Session["acctype"].ToString();
            var query1 = "";
            bool success = false;
            using (var connection = new SqlConnection(ConnectionString))
            {
                if (FpAccntType.Equals("S"))
                {
                    query1 =
                        "SELECT * FROM LOGININFO INNER JOIN STUDENTPROFILE ON LOGININFO.USERNAME = STUDENTPROFILE.USERNAME WHERE (LOGININFO.USERNAME = @username);";
                }
                else if (FpAccntType.Equals("P"))
                {
                    query1 =
                        "SELECT * FROM LOGININFO INNER JOIN FACULTYPROFILE ON LOGININFO.USERNAME = FACULTYPROFILE.USERNAME WHERE (LOGININFO.USERNAME = @username);";
                }
                else if (FpAccntType.Equals("V"))
                {
                    query1 =
                        "SELECT * FROM LOGININFO INNER JOIN VIEWERPROFILE ON LOGININFO.USERNAME = VIEWERPROFILE.USERNAME WHERE (LOGININFO.USERNAME = @username);";
                }
                var command0 = new SqlCommand(query1, connection);
                command0.Parameters.AddWithValue("@username", username);
                try
                {
                    connection.Open();
                    var reader = command0.ExecuteReader();
                    if (reader.Read())
                    {
                        ...
                        var phnNumber = reader.GetString(10);
                        Carrier = reader.GetString(11);
                        var message = "Your two-factor authentication code is: " + ranonNumber;
                        SendSms(Carrier, phnNumber, message);
                        success = true;
                    }
                    else
